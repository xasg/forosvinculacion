<?php
	...
	$total = "SELECT COUNT('mesa') as mesa FROM `asistencia` 
		...
		WHERE usuario.dt_region=$id_region AND `id_cat_mesa`=08 GROUP BY cat_mesas.dt_nombre";
	...
	$html= "<p></p>";
	 ...
	 if($fila['mesa']>=500)
	{
		$html.= "<p style='color:red;'>Mesa 8: llena</p>";
	}else{
		$html.= '<input class="form-check-input" type="radio" name="dual_horario2"  value="08">
