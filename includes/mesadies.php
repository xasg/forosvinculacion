<?php
	...
	$total = "SELECT COUNT('mesa') as mesa FROM `asistencia` 
		...
		WHERE usuario.dt_region=$id_region AND `id_cat_mesa`=10 GROUP BY cat_mesas.dt_nombre";
	...
	$html= "<p></p>";
	 ...
	 if($fila['mesa']>=500)
	{
		$html.= "<p style='color:red;'>Mesa 10: llena</p>";
	}else{
		$html.= '<input class="form-check-input" type="radio" name="dual_horario3" value="10">
                 <label class="form-check-label">Mesa 10. Estructuras estatales y regionales</label>';
	}
