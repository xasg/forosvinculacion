...
function insert_registro($apaterno, $amaterno, $nombre, $email, $tel_ins, $ext, $tel_movil, $region, $entidad, $organizacion, $nom_org, $nom_org2, $cargo, $cargo2, $otro_cargo, $otro_cargo2, $mesa1, $catering,$otra_ies)
{
global $mysqli;
...
dt_cargo2, dt_otro_cargo, dt_otro_cargo2, dt_mesa1, dt_catering,dt_otra_ies) VALUES (null, '{$apaterno}', '{$amaterno}', '{$nombre}', '{$email}', '{$tel_ins}', '{$ext}', '{$tel_movil}', '{$region}', '{$entidad}', '{$organizacion}', '{$nom_org}', '{$nom_org2}', '{$cargo}', '{$cargo2}' , '{$otro_cargo}', '{$otro_cargo2}', '{$mesa1}', '{$catering}', '{$otra_ies}' )";
$mysqli->query($sql);
}
...
function get_region_acept_users($region)
{
  ...
  $sql = "SELECT dt_region, SUM(tp_estatus_conteo) AS aceptados, COUNT(*) AS users FROM usuario where dt_region = {$region}  GROUP BY dt_region;"; //Se agrego centencia a la query que no va a mostrar a estos usuarios, sin tener que modificar la base de datos de produccion
  $result = $mysqli->query($sql);
  return $mysqli->query($sql);
  // $result = $mysqli->query($sql);
  // $limites = $result->fetch_assoc();
  //  return $limite = $limites['aceptados'];  
  # code...
}
function get_region_acept_users_registro($region)
{
  global $mysqli;
  // $sql = "SELECT dt_region, SUM(tp_estatus_conteo) AS aceptados, COUNT(*) AS users FROM usuario where dt_region = '{$region}' GROUP BY dt_region;";
  $sql = "SELECT dt_region, SUM(tp_estatus_conteo) AS aceptados, COUNT(*) AS users FROM usuario where dt_region = {$region}  GROUP BY dt_region;"; //Se agrego centencia a la query que no va a mostrar a estos usuarios, sin tener que modificar la base de datos de produccion
  ...
  $result = $mysqli->query($sql);
  $limites = $result->fetch_assoc();
   return $limite = $limites['users'];  
  # code...
}

function get_limit_region_acept_users($region)
{
  global $mysqli;
  // $sql = "SELECT dt_region, SUM(tp_estatus_conteo) AS aceptados, COUNT(*) AS users FROM usuario where dt_region = '{$region}' GROUP BY dt_region;";
  $sql = "SELECT limite FROM `cat_region` WHERE id_cat_region = '{$region}';"; //Se agrego centencia a la query que no va a mostrar a estos usuarios, sin tener que modificar la base de datos de produccion
  $result = $mysqli->query($sql);
  $limites = $result->fetch_assoc();
   return $limite = $limites['limite']; 
  # code...
}
